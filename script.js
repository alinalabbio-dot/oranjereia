...
document.querySelectorAll('[data-fact]').forEach(b=>b.addEventListener('click',()=>{document.getElementById('kingdomFact').textContent=b.dataset.fact;b.parentElement.querySelectorAll('button').forEach(x=>x.removeAttribute('aria-pressed'));b.setAttribute('aria-pressed','true')}));
...
const choiceGames={
 ...
 visible:{keys:['whale','ant','cat','yogurt','ecoli'],correct:['whale','ant','cat'],success:'Точно! Бактерии можно рассмотреть только с увеличением.'}
};
Object.entries(choiceGames).forEach(([id,g])=>{const box=document.querySelector(`[data-choice-game="${id}"]`);shuffled(g.keys).forEach(k=>box.append(card(k,'choice-card')));box.addEventListener('click',e=>{const c=e.target.closest('.choice-card');if(c)c.classList.toggle('selected')})});
const plantBox=document.querySelector('[data-choice-game="plant"]');const plantItems=[['Солнечный свет','sun',1],['Вода','water',1],['Углекислый газ','air',1],['Колбаса','sausage',0],['Шоколад','chocolate',0],['Гамбургер','burger',0],['Молоко','milk',0]];shuffled(plantItems).forEach(([label,img,ok],i)=>{const el=document.createElement('button');el.className='choice-card';el.dataset.key=`p${i}`;el.dataset.ok=ok;el.innerHTML=`<img src="${asset(img)}" alt=""><span>${label}</span>`;plantBox.append(el)});plantBox.addEventListener('click',e=>{const c=e.target.closest('.choice-card');if(c)c.classList.toggle('selected')});
document.querySelectorAll('[data-check-choice]').forEach(b=>b.addEventListener('click',()=>checkChoice(b.dataset.checkChoice)));
function checkChoice(id){const box=document.querySelector(`[data-choice-game="${id}"]`),selected=[...box.querySelectorAll('.selected')],hint=document.querySelector(`[data-choice-hint="${id}"]`);let ok=false;if(id==='plant'){ok=selected.length===3&&selected.every(x=>x.dataset.ok==='1')}else{const want=choiceGames[id].correct;ok=selected.length===want.length&&selected.every(x=>want.includes(x.dataset.key))}if(ok){selected.forEach(x=>x.classList.add('correct'));hint.textContent=id==='plant'?'Растение приготовило себе еду!':choiceGames[id].success;hint.className='game-hint good';document.querySelector(`[data-choice-next="${id}"]`).classList.remove('hidden');bDisable(id);earn(id);if(id==='plant'){document.querySelector('.plant-stage').classList.add('alive');document.querySelector('[data-plant-slots]').innerHTML='<span>☀</span><span>💧</span><span>CO₂</span>'}}else{selected.forEach(x=>{if(id==='plant'?x.dataset.ok!=='1':!choiceGames[id].correct.includes(x.dataset.key))x.classList.add('wrong')});hint.textContent='Почти! Проверь выбранные карточки и попробуй ещё раз.';hint.className='game-hint try';setTimeout(()=>box.querySelectorAll('.wrong').forEach(x=>x.classList.remove('wrong')),500)}}
function bDisable(id){document.querySelector(`[data-check-choice="${id}"]`).classList.add('hidden')}
document.querySelectorAll('[data-quiz] button').forEach(b=>b.addEventListener('click',()=>{const hint=document.querySelector('[data-quiz-hint="fungi"]');if(b.dataset.answer==='fungi'){b.classList.add('correct');hint.textContent='Верно! Плесень — это тоже гриб. Грибы бывают не только в лесу.';hint.className='game-hint good';document.querySelector('[data-quiz-next="fungi"]').classList.remove('hidden');earn('fungi')}else{b.classList.add('wrong');hint.textContent='Почти! Вспомни, к какому царству относится плесень.';setTimeout(()=>b.classList.remove('wrong'),500)}}));

const rounds=[
 {keys:['wolf','fox','bear','birch'],odd:'birch'}, {keys:['mushroom','mold','yeast','daisy'],odd:'daisy'},
 {keys:['oak','spruce','cactus','yogurt'],odd:'yogurt'}, {keys:['yogurt','ecoli','microscopeB','butterfly'],odd:'butterfly'}
];
